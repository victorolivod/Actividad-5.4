...
@st.cache_data
def cargar_datos():
    return pd.read_excel('df_DataBridgeConsulting (4).xlsx')
...
with tab2:
    ...
    st.markdown("""
    **Inicio**  
    Este dashboard presenta un análisis integral de los pedidos y ventas registrados, permitiendo filtrar por región, tipo de cliente y rango de fechas.
    ...
    **Hallazgos**  
    ...
    - El histograma de volumen revela la concentración de pedidos en ciertos rangos.
    - El gráfico de pedidos por región destaca las zonas con mayor actividad comercial.

    **Análisis**  
    El análisis revela que ciertas regiones y tipos de cliente concentran la mayor parte de las ventas, y que existen temporadas con picos significativos. Además, la distribución de volumen ayuda a identificar oportunidades de optimización logística.
...
# Sube este archivo junto con 'df_DataBridgeConsulting (4).xlsx' a tu repositorio de GitHub para desplegarlo en Streamlit Community Cloud.
